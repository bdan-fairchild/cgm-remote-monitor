...
treatments.html = function html(client) {
  ...
  ret +=
    ...
    + '      </label>'
    + '      <br>'
    + '      <label for="rped_absorptionTime">'
    + translate('Absorption Time')     
    + '          <input type="number" step="any" min="0" id="rped_absorptionTime" placeholder="' + translate('Carbohydrate absorption time') + '" />'
    + '      </label>'
    + '      <br>'
...
treatments.report = function report_treatments(datastorage, sorteddaystoshow, options) {
  ...
  function buildConfirmText(data) {
    ...
    pushIf(data.carbs, translate('Absorption Time') + ': ' + data.absorptionTime);
  ...
  function editTreatment(event) {
	...
	$('#rped_absorptionTime').val(data.absorptionTime ? data.absorptionTime : "");
  ...
  table.append($('<tr>').css('background','gray')
    ...
    .append($('<th>').css('width','50px').attr('align','left').append(translate('Absorption Time')))
  ...
  sorteddaystoshow.forEach(function (day) {
    ...
    for (var t=0; t<treatments.length; t++) {
      ...
      table.append($('<tr>').addClass('border_bottom')
        ...
        .append($('<td>').attr('align','center').append(tr.absorptionTime ? tr.absorptionTime : ''))
        .append($('<td>').attr('align','center').append(tr.duration ? tr.duration : ''))
